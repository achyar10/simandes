                    ...
                    <div class="col-md-9">
                        <div class="form-group">
                            ...
                            <select id="category_id" name="category_id"
                                ...
                                @foreach ($categories as $cat)
                                    <option value="{{ $cat->id }}"
                                        {{ old('category_id', $row->category_id) == $cat->id ? 'selected' : null }}>
                                        {{ $cat->name }}</option>
                                @endforeach
                            </select>
                            @error('category_id')
                        ...
                        <div class="form-group">
                            ...
                            <select id="is_active" name="is_active"
                                ...
                                <option value="1" {{ old('is_active', $row->is_active) == 1 ? 'selected' : null }}>
                                    Publish</option>
                                <option value="0" {{ old('is_active', $row->is_active) == 0 ? 'selected' : null }}>
                                    Draft</option>
                            </select>
                            @error('is_active')
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group">
                            ...
                            @if (!$row->image)
                                <img id="target" src="{{ asset('template/assets/images/placeholder.jpg') }}"
                                    alt="no image" class="img-thumbnail" style="height: 250px; width: 100%">
                            @else
                                <img id="target" src="{{ asset("$uri/$row->image") }}" alt="no image"
                        ...
                        <trix-editor input="x">{!! old('description', $row->description) !!}</trix-editor>
                    </div>
